...
try:
    ...
    ADMIN_LOGIN_PWD = st.secrets["auth"]["admin_login_pwd"] # ⭐️ 新增：管理員認證密碼
except:
...
if user_unit == "總管理員":
    if not st.session_state.admin_verified:
        st.sidebar.warning("🔐 請輸入管理員密碼進行驗證")
        admin_input = st.sidebar.text_input("管理員密碼", type="password", key="admin_auth_box")
        if admin_input == ADMIN_LOGIN_PWD:
            st.session_state.admin_verified = True
            st.rerun()
        else:
            if admin_input != "":
                st.sidebar.error("密碼錯誤，拒絕訪問。")
            st.info("請在左側輸入密碼以解鎖管理員功能。")
            st.stop()
    is_admin = True
else:
    # 當切換回一般單位，重置認證狀態
