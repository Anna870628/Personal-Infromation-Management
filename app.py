...
if menu == "1. 自檢表":
    ...
    edited = st.data_editor(df, num_rows="dynamic", use_container_width=True, column_order=cols, column_config={
        "id": None, 
        "item_no": "🟦項次", "unit_name": st.column_config.TextColumn("🟦單位", disabled=True),
    ...
    st.caption(f"登入身分：{user_unit} (已自動過濾所屬單位資料)")
    ...
    with c2:
        # Excel 匯出順序還原完整名稱
        rename_dict = {"dept_name":"I.部名稱","room_name":"I.室名稱","pi_manager":"I.個資檔案管理者","process_desc":"I.業務流程說明","pi_amount":"II.筆數/份數","pi_purpose":"II.特定目的","pi_category":"II.個資之類別","legal_basis":"II.合法蒐集依據","collect_method":"II.蒐集方式","sys_name":"III.應用系統名稱","sys_source":"III.來源","use_target":"[使用]對象","use_purpose":"[使用]目的","use_method":"[使用]方式","use_protect":"[使用]保護方式","trans_target":"[傳送]對象","trans_purpose":"[傳送]目的","trans_method":"[傳送]方式","trans_protect":"[傳送]保護方式","store_loc":"[儲存]位置","store_legal_time":"[儲存]法定時限","store_inner_time":"[儲存]內部保存期限","store_protect":"[儲存]保護措施","del_method":"[刪除]方式","del_unit":"[刪除]單位","intl_country":"[國際]國家","intl_target":"[國際]對象","intl_purpose":"[國際]目的","intl_method":"[國際]方式","intl_protect":"[國際]保護方式"}
        for s in scopes: rename_dict[f"scope_{s}"] = f"[範圍]{s}"
        rules = {
            "blue": ["I.部名稱","I.室名稱","I.個資檔案管理者","I.業務流程說明"],
            "green": ["II.筆數/份數","II.特定目的","II.個資之類別","II.合法蒐集依據","II.蒐集方式"] + [f"[範圍]{s}" for s in scopes],
            "orange": ["III.應用系統名稱","III.來源","[使用]對象","[使用]目的","[使用]方式","[使用]保護方式","[傳送]對象","[傳送]目的","[傳送]方式","[傳送]保護方式"],
            "purple": ["[儲存]位置","[儲存]法定時限","[儲存]內部保存期限","[儲存]保護措施","[刪除]方式","[刪除]單位"],
    ...
    edited = st.data_editor(df, num_rows="dynamic", use_container_width=True, column_order=cols, column_config={
        "id": None, "unit_name": st.column_config.TextColumn("🟦單位", disabled=True), "project_name": "🟦業務名稱",
        "score_1":"🟨(1)數量", "score_2":"🟨(2)敏感度", "score_3":"🟨(3)信譽損害", "score_4":"🟨(4)隱私衝擊", "score_5":"🟨(5)合作單位"
    })
    if st.button("💾 儲存評估"):
    ...
    edited = st.data_editor(df, num_rows="dynamic", use_container_width=True, column_order=cols, column_config={
        "id": None, "unit_name": st.column_config.TextColumn("🟦單位", disabled=True), "vendor_name": "🟦廠商名稱", "file_name": "🟦檔案名稱",
